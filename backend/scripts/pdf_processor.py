...
def dispatch_tool(tool_id, input_paths, output_path):
...
def main():
    ...
    # Check if this is a bulk operation that needs to be zipped
    is_merge = tool_id in ["merge-pdf", "merge"]

    try:
        if len(input_paths) > 1 and not is_merge:
            import tempfile
            import zipfile
            
            print(f"Running bulk zip operation for {len(input_paths)} files...")
            with tempfile.TemporaryDirectory() as tmpdir:
                ...
                for index, input_path in enumerate(input_paths):
                    ...
                    ext = ".pdf"
                    if tool_id in ["pdf-to-word", "pdf-word"]:
                        ext = ".docx"
                    elif tool_id in ["pdf-to-excel", "pdf-excel"]:
                        ext = ".xlsx"
                    elif tool_id in ["pdf-to-ppt", "pdf-ppt"]:
                        ext = ".pptx"
                    elif tool_id in ["pdf-to-txt", "pdf-txt", "pdf-to-text"]:
                        ext = ".txt"
                    elif tool_id in ["pdf-to-html", "pdf-html"]:
                        ext = ".html"
                    elif tool_id in ["pdf-to-png", "pdf-png", "split-pdf", "split"]:
                        ext = ".zip"
                        
                    input_stem = input_path.stem
                    tmp_out_name = f"{input_stem}{ext}"
                    # Avoid filename collisions in bulk folder
                    if (tmpdir_path / tmp_out_name).exists():
                        tmp_out_name = f"{input_stem}_{index}{ext}"
                        
                    tmp_output_path = tmpdir_path / tmp_out_name
                    
                    print(f"Processing bulk item [{index + 1}/{len(input_paths)}]: {input_path.name}")
                    dispatch_tool(tool_id, [input_path], tmp_output_path)
                    
                    if tmp_output_path.exists():
                        processed_files.append((tmp_output_path, tmp_out_name))
                
                if not processed_files:
                    raise Exception("All bulk items failed to process.")
                    
                # Create the final output zip file
                with zipfile.ZipFile(output_path, "w", zipfile.ZIP_DEFLATED) as zip_file:
                    for file_path, archive_name in processed_files:
                        zip_file.write(file_path, archive_name)
                        
            print(f"Bulk zip package created successfully at {output_path}")
        else:
            # Single-file standard operation
            dispatch_tool(tool_id, input_paths, output_path)
