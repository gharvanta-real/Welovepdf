...
const seoPages: Record<string, SeoPage> = {
  merge: {
    ...
    steps: ["Select or drag and drop the PDF files you wish to merge.", "Rearrange the order of the uploaded documents by dragging them left or right.", "Click the 'Start' button to execute the merge operation.", "Download your combined PDF instantly."],
    faqs: [{ q: "How many files can I merge at once?", a: "Starter tier users can merge up to 5 files (up to 50 MB total) per session, with 10 tool uses per day. Pro tier users can combine up to 100 documents of up to 500 MB." }, { q: "Will my merged PDF have a watermark?", a: "No, PDFMount does not append any watermarks or brand markings to your documents. All processed files remain clean and professional." }, { q: "Is it safe to merge confidential documents here?", a: "Absolutely. All processing runs under strict secure isolation on our server, and temporary workspace files are fully purged from storage after 1 hour." }],
    detailedContent: ["Whether you are compiling monthly business reports, organizing student coursework, or combining tax documents, having scattered files makes tracking difficult.", "PDFMount's interface lets you visually drag and drop files to rearrange page orders before compiling."],
  },
  split: {
    ...
    steps: ["Upload the PDF document you want to split.", "Input the custom page ranges (e.g. 1-3, 5) or choose specific pages to extract.", "Click the 'Start' button to divide the document.", "Save your separate PDF pages directly."],
    faqs: [{ q: "Can I split a PDF into separate single-page files?", a: "Yes, you can extract individual pages by defining single-page ranges (e.g. 1, 2, 3) to split your document." }, { q: "What is the difference between split ranges and extract?", a: "Split ranges splits the PDF at custom boundaries into multiple files, while Extract saves only selected pages into a single new document." }],
    detailedContent: ["Large documents can be slow to email and hard to navigate. By splitting your PDF, you can discard redundant sections and focus solely on the critical pages.", "This tool is highly optimized for lawyers, accountants, and administrators who frequently need to extract single pages or specific chapters from large contracts."],
  },
  compress: {
    ...
    steps: ["Choose the PDF file you wish to compress.", "Select your preferred compression level (Recommended, Extreme, or Less Compression).", "Click the 'Start' button to execute compression.", "Download your optimized PDF file."],
    faqs: [{ q: "Does compressing a PDF ruin its image quality?", a: "No. Our recommended setting uses a smart compression algorithm that reduces file size dramatically while maintaining high visual readability." }, { q: "How can I reduce a PDF to under 100KB?", a: "Try selecting the 'Extreme Compression' setting. This applies strong optimization suitable for basic text sheets and scanned forms." }],
    detailedContent: ["Many government portals, job application forms, and email clients enforce strict size limits. PDFMount's compression tool uses advanced algorithms to compress embedded images, remove redundant metadata, and clean up orphan streams.", "For professional portfolios and legal briefs, choose 'Less Compression' to keep image DPI high while stripping out underlying bloat."],
  },
  "jpg-pdf": {
  ...
  "pdf-annotator": {
    ...
    steps: ["Select and upload the PDF file.", "Use drawing and text tools in the workspace editor to annotate pages.", "Click the 'Start' button to save annotations.", "Save your annotated PDF."],
    faqs: [{ q: "Are annotations editable after downloading?", a: "Yes, standard PDF readers will recognize highlights and text comments as editable document markup." }],
    detailedContent: ["Reviewing drafts with client feedback is simple. Our annotator embeds markup vectors onto pages without changing the original text blocks."],
  },
  "header-footer-pdf": {
